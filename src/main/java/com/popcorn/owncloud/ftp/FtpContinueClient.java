package com.popcorn.owncloud.ftp;
...
public class FtpContinueClient {
    ...
    public UploadStatus upload(String local, String remote)
        ...
        // 检查远程是否存在文件
        FTPFile[] files = ftpClient.listFiles(new String(remoteFileName.getBytes(StandardCharsets.UTF_8), "iso-8859-1"));
        if (files.length == 1) {
    ...
    public UploadStatus CreateDirecroty(String remote, FTPClient ftpClient)
        ...
        if (!directory.equalsIgnoreCase("/")
                && !ftpClient.changeWorkingDirectory(new String(directory.getBytes(StandardCharsets.UTF_8), "iso-8859-1"))) {
            ...
            while (true) {
                String subDirectory = new String(remote.substring(start, end).getBytes(StandardCharsets.UTF_8), "iso-8859-1");
    ...
    public UploadStatus uploadFile(String remoteFile, File localFile, FTPClient ftpClient, long remoteSize)
        ...
        OutputStream out = ftpClient.appendFileStream(new String(remoteFile.getBytes(StandardCharsets.UTF_8), "iso-8859-1"));
        // 断点续传
        if (remoteSize > 0) {
